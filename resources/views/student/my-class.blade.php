                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                            <svg class="w-5 h-5 text-islamic-emerald" viewBox="0 0 256 256" fill="currentColor">
                    ...
                    <div class="bg-gradient-to-br from-islamic-emerald to-islamic-emerald-dark rounded-xl shadow-lg p-6 text-white relative overflow-hidden">
                        ...
                        @if($subscription->assignedTeacher)
                            @php
                                $teacher = $subscription->assignedTeacher;
                                $teacherPhone = preg_replace('/^0/', '62', $teacher->phone ?? '');
                                $teacherPhone = preg_replace('/[^0-9]/', '', $teacherPhone);
                            @endphp
                            <div class="flex items-center gap-4 relative z-10">
                                <div class="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center text-xl font-bold border-2 border-white/30">
                                    {{ strtoupper(substr($teacher->name, 0, 1)) }}
                                </div>
                                <div>
                                    <div class="font-bold text-lg">{{ $teacher->name }}</div>
                                    <div class="text-sm text-islamic-emerald-light">
                                        {{ $teacher->gender === 'male' ? 'Ustadz' : 'Ustadzah' }}
                                    </div>
                                </div>
                            </div>
                            
                            <div class="mt-6 pt-4 border-t border-white/10 relative z-10">
                                <a href="https://wa.me/{{ $teacherPhone }}?text=Assalamu'alaikum%20{{ urlencode($teacher->name) }},%20saya%20{{ urlencode(auth()->user()->name) }}%20dari%20kelas%20{{ urlencode($enrolledClass->name) }}" 
                                   target="_blank"
                                   class="flex items-center justify-center gap-2 w-full py-3 px-4 bg-white/20 hover:bg-white/30 text-white font-medium rounded-xl transition-colors">
                        ...
                        @else
                            <div class="flex items-center gap-4 relative z-10">
                                ...
                                <div>
                                    <div class="font-bold text-lg">Menunggu Penunjukan</div>
                                    <div class="text-sm text-islamic-emerald-light">Admin akan segera menunjuk pengajar</div>
                                </div>
                            </div>
                        @endif
                    </div>
