                    ...
                    <div>
                        ...
                        <div class="space-y-1">
                            ...
                            @else
                                ...
                                <a href="{{ route('admin.reports.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-xl {{ request()->routeIs('admin.reports.*') ? 'text-red-600 font-bold bg-red-50' : 'text-gray-500 hover:text-gray-900' }} transition-all">
                                    <svg class="w-5 h-5 {{ request()->routeIs('admin.reports.*') ? 'text-red-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                    <span>Laporan</span>
                                </a>
                                
                                <a href="{{ route('admin.attendances.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-xl {{ request()->routeIs('admin.attendances.*') ? 'text-red-600 font-bold bg-red-50' : 'text-gray-500 hover:text-gray-900' }} transition-all">
                                    <svg class="w-5 h-5 {{ request()->routeIs('admin.attendances.*') ? 'text-red-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                    ...
                    <div class="flex items-center gap-3 p-3 rounded-2xl bg-gray-50">
                        <div class="w-10 h-10 rounded-full flex-shrink-0 overflow-hidden bg-red-100 flex items-center justify-center text-red-600 font-bold">
                            @php
                                ...
                                $hasPhoto = !empty($photoPath) && \Illuminate\Support\Facades\Storage::disk('public')->exists($photoPath);
                            @endphp
                            @if($hasPhoto)
                                <img src="{{ asset('storage/' . $photoPath) }}" alt="{{ auth()->user()->name }}" class="w-full h-full object-cover">
                            @else
